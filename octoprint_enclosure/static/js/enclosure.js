$(function() {
  function EnclosureViewModel(parameters) {
    ...
    self.settings_outputs_regular = ko.pureComputed(function() {
      return ko.utils.arrayFilter(
        self.settingsViewModel.settings.plugins.enclosure.rpi_outputs(),
        function(item) {
    ...
    self.settings_possible_outputs = ko.pureComputed(function() {
      return ko.utils.arrayFilter(
        ...
        function(item) {
          return (
            (item.output_type() === "regular" && !item.toggle_timer()) ||
            item.output_type() === "gcode_output" ||
            item.output_type() === "shell_output"
          );
        }
      );
    });
    ...
    self.settings_temperature_sensors = ko.pureComputed(function() {
      return ko.utils.arrayFilter(
        self.settingsViewModel.settings.plugins.enclosure.rpi_inputs(),
        function(item) {
          return item.input_type() === "temperature_sensor";
        }
      );
    ...
    self.settings_hum_sensors = ko.pureComputed(function() {
      return ko.utils.arrayFilter(self.settings_temperature_sensors(), function(
        sensor
      ) {
    ...
    self.humidityCapableSensor = function(sensor) {
      if (
        ["11", "22", "2302", "bme280", "si7021", "sensors"].indexOf(sensor) >= 0
      ) {
        return true;
      }
      return false;
    ...
    self.isRegularOutput = function(index_id) {
      ...
      if (typeof index_id != "undefined") {
        self.settingsViewModel.settings.plugins.enclosure
          .rpi_outputs()
          .forEach(function(output) {
            if (
              output.index_id() == index_id &&
              output.output_type() == "regular"
            ) {
              return_value = true;
              return false;
            }
      ...
      return ko.pureComputed(function() {
        return ko.utils.arrayFilter(self.rpi_outputs(), function(item) {
          if (item.linked_temp_sensor) {
            return (
              item.linked_temp_sensor() == sensor_index &&
    ...
    self.hasAnyNavbarOutput = function() {
      ...
      self.rpi_outputs().forEach(function(output) {
        if (
          (output.output_type() == "regular" ||
            output.output_type() == "gcode_output") &&
          output.show_on_navbar()
        ) {
          return_value = true;
          return false;
        }
    ...
    self.onDataUpdaterPluginMessage = function(plugin, data) {
      ...
      if (data.hasOwnProperty("sensor_data")) {
        data.sensor_data.forEach(function(sensor_data) {
          var linked_temp_sensor = ko.utils
            .arrayFilter(self.rpi_inputs_temperature_sensors(), function(
              temperature_sensor
            ) {
      ...
      if (data.hasOwnProperty("set_temperature")) {
        data.set_temperature.forEach(function(set_temperature) {
          var linked_temp_control = ko.utils
            .arrayFilter(self.rpi_outputs(), function(temp_control) {
              return set_temperature["index_id"] == temp_control.index_id();
            })
            .pop();
          if (linked_temp_control) {
            linked_temp_control.temp_ctr_set_value(
      ...
      if (data.hasOwnProperty("rpi_output_regular")) {
        data.rpi_output_regular.forEach(function(output) {
          var linked_output = ko.utils
            .arrayFilter(self.rpi_outputs(), function(item) {
      ...
      if (data.hasOwnProperty("filament_sensor_status")) {
        data.filament_sensor_status.forEach(function(filament_sensor) {
          var linked_filament_sensor = ko.utils
            .arrayFilter(self.rpi_inputs(), function(item) {
              return filament_sensor["index_id"] == item.index_id();
            })
            .pop();
          if (linked_filament_sensor) {
            linked_filament_sensor.filament_sensor_enabled(
    ...
    self.isOperational = ko.computed(function() {
      return (
        self.connectionViewModel.loginState.isUser() &&
        self.printerStateViewModel.isOperational()
      );
    });
    ...
    self.bindFromSettings = function() {
      ...
      self.use_sudo(
        self.settingsViewModel.settings.plugins.enclosure.use_sudo()
      );
      self.gcode_control(
        self.settingsViewModel.settings.plugins.enclosure.gcode_control()
      );
      self.neopixel_dma(
        self.settingsViewModel.settings.plugins.enclosure.neopixel_dma()
      );
      self.debug(self.settingsViewModel.settings.plugins.enclosure.debug());
      self.debug_temperature_log(
        self.settingsViewModel.settings.plugins.enclosure.debug_temperature_log()
      );
      self.use_board_pin_number(
        self.settingsViewModel.settings.plugins.enclosure.use_board_pin_number()
      );
      self.filament_sensor_gcode(
        self.settingsViewModel.settings.plugins.enclosure.filament_sensor_gcode()
      );
      self.notification_provider(
        self.settingsViewModel.settings.plugins.enclosure.notification_provider()
      );
      self.notification_event_name(
        self.settingsViewModel.settings.plugins.enclosure.notification_event_name()
      );
      self.notification_api_key(
        self.settingsViewModel.settings.plugins.enclosure.notification_api_key()
      );
      self.notifications(
        self.settingsViewModel.settings.plugins.enclosure.notifications()
      );
    };
    ...
    self.setTemperature = function(item, form) {
      ...
      if (self.isNumeric(newSetTemperature)) {
        var request = {
          set_temperature: newSetTemperature,
    ...
    self.addRpiOutput = function() {
      var arrRelaysLength = self.settingsViewModel.settings.plugins.enclosure.rpi_outputs()
        .length;
    ...
    self.removeRpiOutput = function(data) {
      self.settingsViewModel.settings.plugins.enclosure.rpi_outputs.remove(
        data
      );
    };

    self.addRpiInput = function() {
      var arrRelaysLength = self.settingsViewModel.settings.plugins.enclosure.rpi_inputs()
        .length;
    ...
    self.removeRpiInput = function(definition) {
      self.settingsViewModel.settings.plugins.enclosure.rpi_inputs.remove(
        definition
      );
    };
    ...
    self.handleNeopixel = function(item) {
      ...
      if (
        ...
        new PNotify({
          title: "Enclosure",
          text:
            "Color needs to follow the format rgb(value_red,value_green,value_blue)!",
          type: "error"
        });
      } else {
        $.ajax({
  ...
  OCTOPRINT_VIEWMODELS.push({
    ...
    elements: [
      "#tab_plugin_enclosure",
      "#settings_plugin_enclosure",
      "#navbar_plugin_enclosure_1",
      "#navbar_plugin_enclosure_2"
    ]
  });
});
